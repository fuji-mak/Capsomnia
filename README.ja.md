...
- 手動状態の使用中はCaps Lock LEDを10ミリ秒ごとに確認し、macOSに上書きされた場合だけ再設定します。再点灯まで一瞬消える場合があります。
- LED制御にはmacOSの非公開`HIDCapsLockLED`プロパティを使用するため、将来のmacOSでは動作しなくなる可能性があります。
...
スリープ制御と手動LED出力に入力監視の許可は不要です。Capsomniaはキーボードデバイスを開いたり入力イベントを読んだりせず、ローカルのCaps Lock状態とLEDプロパティだけを確認します。LEDプロパティが利用できなくても、スリープ制御とメニューバー表示は動作します。
