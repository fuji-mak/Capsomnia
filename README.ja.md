...
- Caps Lock ON中に蓋を閉じた時: `pmset displaysleepnow` を実行し、作業を走らせたまま画面だけスリープ
- メニューバーの黄緑の丸: スリープ抑止中
- メニューバーのグレーの丸: 通常のスリープ動作
- 初回起動時の設定: メニューバーの丸を表示するか、日本語・英語のどちらを使うかを選択
- 設定画面: メニューバー表示、蓋閉じ時の画面オフ、言語、ログイン時起動を切り替え
- アプリ終了時は通常のスリープ動作へ戻す
...
初回起動時に小さな初期設定画面が開き、メニューバーの丸を表示するか、日本語・英語のどちらを使うかを選べます。あとからメニューバー表示、蓋閉じ時の画面オフ、言語、ログイン時起動を変更する場合は Capsomnia をもう一度開きます。
