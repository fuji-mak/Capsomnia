...
主な変更点は、ネイティブのメニューバー UI、直接操作できる「有効」スイッチ、中国語・英語・日本語対応、蓋を閉じている間の確実な画面スリープ、インストールと状態確認の強化です。主操作だけを目立たせ、補助設定は標準のチェックマークで表示します。状態は塗りつぶし円と中空円、エラーは赤で示します。

この 1.3.1 カスタム版は、ソースまたはローカルでビルドした未署名パッケージ（payload は ad-hoc 署名）だけで提供します。原作者の Developer ID や Apple 公証は使用していません。公式 1.0.0 の署名・公証はこのカスタム版には適用されません。

**Capsomnia** は、MacBook の蓋を閉じてもローカル処理を止めないための小さな macOS アプリです。

作業を走らせ続けたいときは「有効」をオン。通常のスリープ動作に戻したいときはオフにします。
...
1. ローカルでビルドするか、このカスタムソースと一緒に提供された `Capsomnia-1.3.1-cn-unsigned.pkg` を使用する
...
パッケージで配置するアプリ、helper、システムLaunchAgentは`root:wheel`所有です。ローカル未署名ビルドではアプリとhelperをad-hoc署名で封印し、Developer ID署名は別途設定した正式配布ビルドでのみ使用します。Capsomniaは切替直後と以後10秒ごとに実際の`SleepDisabled`状態を確認します。helperが変更できない、状態を確認できない、設定が外部要因でずれた場合は、要求した状態を有効として表示せず、メニューバーの丸を赤色にして5秒後に再同期します。通常メニューバー表示を隠している場合も、エラー中は赤い丸を一時表示します。
...
Capsomnia 1.3.1では「有効」だけに目立つスイッチを残し、二つの補助機能はネイティブのチェックマークで表示します。リリース履歴は [CHANGELOG.md](CHANGELOG.md)、脆弱性報告の方針は [SECURITY.md](SECURITY.md) を参照してください。
