...
这个 fork 已跟随上游 4.0，包括 CLI、Skill、更新检查和发行工具，同时将唤醒控制改为明确的电源策略：

- **保持唤醒**会保存用户期望的状态，不再把 Caps Lock 当作电源开关。
- **电池供电时自动关闭唤醒**默认开启。使用电池时，Capsomnia 会解除防休眠，但不会终止正在运行的进程。
- 重新接入电源后，会自动恢复此前保存的保持唤醒状态。
- 菜单勾选、状态圆点、标题、提示、CLI 状态和 doctor 输出均反映实际生效状态。
- 全局快捷键和 `cpsm on/off/toggle` 操作与菜单相同的持久化请求。
- 登录时启动、自动关闭定时器、显示常亮、权限受限的辅助程序、CLI/Skill 安装及手动更新检查均保留上游 4.0 能力。

电池策略只会暂时覆盖保存的请求，不会将其清除。个人 fork 默认关闭每日自动更新检查，因为安装上游软件包会覆盖二开行为。下方上游文档中提到以 Caps Lock 作为唤醒开关的部分，在本 fork 中均由菜单、快捷键和 CLI 请求取代。
...
“登录时启动”默认开启，不会显示在初始设置中。之后再次打开Capsomnia时，可以直接操作日常更常用的“保持显示屏常亮”和自动关闭定时器。“保持显示屏常亮”默认关闭，因此通常合盖后显示屏会进入睡眠。定时器也默认关闭。每次开启Capsomnia时都会从所选时长重新开始，也可以使用重启按钮重置当前倒计时。高级设置集中放置较少修改的菜单栏显示、防止大写锁定、语言、登录时启动、合盖期间的Caps Lock保护和全局快捷键。启用“防止输入锁定为大写”时，“显示菜单栏图标”仍可独立开关。即使隐藏菜单栏图标，发生错误时也会暂时显示红色圆点。
