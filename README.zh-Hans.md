...
“合盖时关闭显示屏”和“登录时启动”默认开启，不会显示在初始设置中。之后可以再次打开Capsomnia，修改包括这两项在内的所有设置。还可以在高级设置中录制用于切换真实Caps Lock状态的全局快捷键。启用“防止输入锁定为大写”时，“显示菜单栏图标”仍可独立开关。即使隐藏菜单栏图标，发生错误时也会暂时显示红色圆点。
