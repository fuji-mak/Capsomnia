...
struct AppStrings {
    ...
    let autoOffRestartOnReenable: String
    let autoOffRestartOnReenableDesc: String
    let autoOffShowInMenuBar: String
    let autoOffShowInMenuBarDesc: String
    ...
    static func localized(for language: AppLanguage) -> AppStrings {
        ...
        case .english:
            AppStrings(
                ...
                autoOffTimerDesc: "Automatically turn awake mode off after a set time. When off, awake mode stays on until you turn Caps Lock off yourself.",
                autoOffOff: "Off",
                ...
                autoOffRestartOnReenable: "Restart timer on re-enable",
                autoOffRestartOnReenableDesc: "When on, turning Caps Lock off and back on restarts the full auto-off duration. When off, the countdown continues where it left off.",
                autoOffShowInMenuBar: "Show timer in menu bar",
                autoOffShowInMenuBarDesc: "Show the remaining auto-off time next to the menu bar icon while it counts down.",
        ...
        case .korean:
            AppStrings(
                ...
                autoOffTimerDesc: "설정한 시간이 지나면 절전 방지를 자동으로 끕니다. 꺼두면 Caps Lock을 직접 끌 때까지 유지됩니다.",
                autoOffOff: "끄기",
                ...
                autoOffRestartOnReenable: "다시 켤 때 타이머 재시작",
                autoOffRestartOnReenableDesc: "켜면 Caps Lock을 껐다가 다시 켤 때 자동 종료 시간을 처음부터 다시 셉니다. 끄면 남은 시간에서 이어서 계속됩니다.",
                autoOffShowInMenuBar: "메뉴 막대에 타이머 표시",
                autoOffShowInMenuBarDesc: "카운트다운 중에 남은 자동 종료 시간을 메뉴 막대 아이콘 옆에 표시합니다.",
        ...
        case .japanese:
            AppStrings(
                ...
                autoOffTimerDesc: "設定した時間が経過すると、スリープ抑止を自動的にオフにします。オフのときは、Caps Lockを自分でオフにするまで維持します。",
                autoOffOff: "オフ",
                autoOffCustom: "カスタム",
                ...
                autoOffRestartOnReenable: "再オンでタイマーを再スタート",
                autoOffRestartOnReenableDesc: "オンにすると、Caps Lockをオフにして再びオンにしたとき、自動オフの時間を最初から数え直します。オフのときは、残り時間の続きから再開します。",
                autoOffShowInMenuBar: "メニューバーにタイマーを表示",
                autoOffShowInMenuBarDesc: "カウントダウン中、残りの自動オフ時間をメニューバーのアイコン横に表示します。",
        ...
        case .simplifiedChinese:
            AppStrings(
                ...
                autoOffTimerDesc: "在设定的时间后自动关闭防睡眠。关闭时，将保持开启，直到你自行关闭 Caps Lock。",
                autoOffOff: "关闭",
                autoOffCustom: "自定义",
                ...
                autoOffRestartOnReenable: "重新开启时重置计时器",
                autoOffRestartOnReenableDesc: "开启后，关闭再打开 Caps Lock 会重新计算完整的自动关闭时长。关闭后，倒计时将从上次剩余处继续。",
                autoOffShowInMenuBar: "在菜单栏显示计时器",
                autoOffShowInMenuBarDesc: "倒计时期间，在菜单栏图标旁显示剩余的自动关闭时间。",
...
private enum PreferenceKey {
    ...
    static let autoOffRestartOnReenable = "AutoOffRestartOnReenable"
    static let autoOffShowInMenuBar = "AutoOffShowInMenuBar"
...
enum Preferences {
    ...
    static func registerDefaults() {
        defaults.register(defaults: [
            ...
            PreferenceKey.autoOffRestartOnReenable: true,
            PreferenceKey.autoOffShowInMenuBar: true,
    ...
    /// When `true`, turning Caps Lock off and back on restarts the full auto-off
    /// duration (old behavior). When `false` (default), the countdown pauses
    /// while off and resumes where it left off.
    static var autoOffRestartOnReenable: Bool {
        get { defaults.bool(forKey: PreferenceKey.autoOffRestartOnReenable) }
        set { defaults.set(newValue, forKey: PreferenceKey.autoOffRestartOnReenable) }
    }

    /// Whether the remaining auto-off time is shown next to the menu bar icon.
    static var autoOffShowInMenuBar: Bool {
        get { defaults.bool(forKey: PreferenceKey.autoOffShowInMenuBar) }
        set { defaults.set(newValue, forKey: PreferenceKey.autoOffShowInMenuBar) }
    }
