...
**合上 Mac，AI 继续跑；任务结束，Mac 自己睡。** 自动睡眠默认开启，任务完成后会等待 30 秒，期间可以从菜单取消本次睡眠。
...
1. 在本机完成构建，或取得与本定制源码一同交付的 `Capsomnia-1.4.0-cn-unsigned.pkg`。
2. 打开安装包，并按照安装器提示完成安装；如 macOS 提示来源未验证，请按你的安全策略确认来源后继续。

本定制版 1.4.0 只能交付本地构建的未签名安装包（payload 使用 ad-hoc 签名）或源码，未使用原作者的 Developer ID，也未经 Apple 公证。它会将 `Capsomnia.app` 安装到 `/Applications`，安装原生特权 helper、受限的 sudoers 规则和 LaunchAgent。安装后 Capsomnia 会自动打开；之后每次登录也会自动启动。
...
- **Codex/Claude 任务完成：** 默认等待 30 秒后让 Mac 自动睡眠；“取消本次自动睡眠”只取消这一次，不会关闭长期设置。
...
Capsomnia 1.4.0 采用单一原生菜单栏入口，只为“合盖时保持开机状态”保留醒目开关；自动睡眠和其他次要功能使用原生对勾。发布历史请查看 [CHANGELOG.md](CHANGELOG.md)，安全模型和漏洞报告方式请查看 [简体中文安全说明](SECURITY.zh-CN.md)。
