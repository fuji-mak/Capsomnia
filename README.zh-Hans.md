...
- 防止输入锁定为大写（可选）：指示灯亮起时，防止输入被锁定为大写。仍可按住Shift输入大写字母。
...
“合盖时关闭显示屏”和“登录时启动”默认开启，不会显示在初始设置中。之后可以再次打开Capsomnia，修改包括这两项在内的所有设置。启用“防止输入锁定为大写”时，“显示菜单栏图标”仍可独立开关。即使隐藏菜单栏图标，发生错误时也会暂时显示红色圆点。
