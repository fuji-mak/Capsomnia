...
- 保持显示屏常亮（可选）：Capsomnia开启期间，显示屏不会在macOS设定的空闲时间后关闭。默认关闭；合盖时仍可正常关闭显示屏。
- 退出应用：恢复正常睡眠行为。
...
“合盖时关闭显示屏”和“登录时启动”默认开启，不会显示在初始设置中。之后可以再次打开Capsomnia，修改包括这两项在内的所有设置。高级设置中可以配置可选的自动关闭定时器、可选的“保持显示屏常亮”设置，以及用于切换真实Caps Lock状态的全局快捷键。定时器默认关闭。每次开启Capsomnia时都会从所选时长重新开始，也可以使用重启按钮重置当前倒计时。启用“防止输入锁定为大写”时，“显示菜单栏图标”仍可独立开关。即使隐藏菜单栏图标，发生错误时也会暂时显示红色圆点。
