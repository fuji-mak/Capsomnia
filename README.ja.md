...
- メニューバーの「スリープを防止」: Caps LockをControlなどに変更している場合も、チェックのオン・オフで同じ操作ができ、HID出力を利用できる場合は内蔵Caps Lock LEDも同期します。物理Caps Lockの状態が変わると、元の物理スイッチ操作へ戻ります。
...
- 入力ソースやキーボード状態が変わると、macOSが手動設定したCaps Lock LEDを上書きする場合があります。この変更は手動状態の切替時にLEDを同期しますが、消灯後の常時再点灯は行いません。
...
スリープ制御に入力監視の許可は不要です。Capsomniaはキーボードイベントを読まず、ローカルのCaps Lock状態だけを250ミリ秒ごとに確認します。macOSやキーボードの構成によっては、手動LED出力のために「システム設定 > プライバシーとセキュリティ > 入力監視」でCapsomniaの許可が必要になる場合があります。LED出力を拒否されても、スリープ制御とメニューバー表示は動作します。
...
Capsomniaはキーボードイベントを読み取りません。ローカルのCaps Lock状態だけを250ミリ秒ごとに確認し、macOSがタイマー起床をまとめられるようtoleranceを設定しています。メニューから手動操作した場合は、スリープ設定の反映を確認してから標準HIDのCaps Lock LED出力を書き込みます。LED書き込みの失敗は回数を制限して再試行し、スリープ制御には影響させません。
